...
# The above code is a Python comment. Comments in Python start with the `#` symbol and are used to
# provide explanations or notes within the code. Comments are ignored by the Python interpreter and
...
1. **Getting phone number from the personal details**
   - **Phone number:** If the question is asking for phone number, string phone_prefix is +33 then give the phone number adding prefix 0 with phone number for any other prefix give number like without adding prefix.
...
When responding, consider all available information, including personal_information, projects, work experience, and academic background, to provide an accurate and well-reasoned answer. Make every effort to infer relevant experience and avoid defaulting to 0 if any related experience can be estimated.
