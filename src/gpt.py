...
class LoggerChatModel:
    ...
    #     return reply
    def __call__(self, prompt_value: ChatPromptValue) -> str:
        # Check if prompt_value is an instance of ChatPromptValue
        if isinstance(prompt_value, ChatPromptValue):
            # Extract the messages list from ChatPromptValue
            messages = prompt_value.messages
        else:
            # If prompt_value is not a ChatPromptValue, use it directly (assuming it's already a list of messages)
            messages = prompt_value

        # Ensure `messages` is a list of message objects (HumanMessage or AIMessage)
        if isinstance(messages, list) and all(isinstance(msg, (HumanMessage, AIMessage)) for msg in messages):
            # Now you have a list of HumanMessage or AIMessage objects
            reply = self.llm(messages)
        else:
            raise TypeError(f"Got unknown type {messages}")
...
class GPTAnswerer:
    def __init__(self, openai_api_key):
        self.llm_cheap = LoggerChatModel(
            ...
            ChatGroq(model_name="llama3-70b-8192", groq_api_key=openai_api_key, temperature=0.4)
        )
    @property
    ...
    def answer_question_numeric(self, question: str, default_experience: int = 3) -> int:
        ...
        chain = prompt | self.llm_cheap | StrOutputParser()
        output_str = chain.invoke({"resume_personal_information": self.resume.personal_information,"resume_educations": self.resume.education_details,"resume_jobs": self.resume.experience_details,"resume_projects": self.resume.projects , "question": question})
        try:
    ...
    def get_phone_number(self, question: str, default_experience: int = 3) -> int:
        if self.resume.personal_information.phone_prefix == "+33":
            return f"0{self.resume.personal_information.phone}"
        return self.resume.personal_information.phone
